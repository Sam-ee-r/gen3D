  ...
  return (
    <div className="flex flex-col h-full bg-tech-bg/40 backdrop-blur-md rounded-2xl border border-tech-border overflow-hidden shadow-xl">
      ...
      <div className="flex border-b border-tech-border shrink-0 bg-black/5">
        ...
        <button
          onClick={() => handleTabChange("meshSettings")}
          className={`flex-1 py-2 text-[10px] font-mono font-semibold uppercase tracking-wider flex items-center justify-center gap-1.5 border-b-2 transition-all cursor-pointer ${
            activeStudioTab === "meshSettings"
              ? "border-primary text-primary bg-primary/5"
              : "border-transparent text-tech-muted hover:text-tech-fg"
          }`}
        >
        ...
        {activeStudioTab === "meshSettings" && (
          <div className="flex-1 flex flex-col">
            ...
            {allMaterials.length > 0 && (
              <div className="px-4 py-3 border-b border-tech-border/50 shrink-0 bg-black/10">
                <p className="text-[9px] font-mono text-tech-muted uppercase tracking-widest mb-2">
                  Model Materials ({allMaterials.length})
                </p>
                <div className="flex flex-wrap gap-2">
                  {allMaterials.map((mat, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        title={mat.name || `Material ${i}`}
                        onClick={() => selectMaterial(mat, i)}
                        className={`relative group flex flex-col items-center gap-1 cursor-pointer transition-all duration-200`}
                      >
                        <div
                          className={`w-8 h-8 rounded-lg border-2 transition-all duration-200 shadow-md ${
                            isActive
                              ? "border-primary scale-110 shadow-[0_0_10px_oklch(0.7_0.2_200/0.5)]"
                              : "border-white/10 hover:border-white/30 hover:scale-105"
                          }`}
                          style={{ backgroundColor: hexColor }}
                        />
                        <span className="text-[8px] font-mono text-tech-muted max-w-[32px] truncate">
                          {mat.name ? mat.name.slice(0, 5) : `M${i}`}
                        </span>
                      </button>
                    );
                  })}
            ...
            {selectedMaterial ? (
              <div className="flex flex-col gap-5 px-4 py-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono text-tech-fg font-semibold truncate">
                    {selectedMaterial.name || "Unnamed Material"}
                  </span>
                </div>

                {/* ── Base Tint ── */}
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] font-mono text-tech-muted uppercase tracking-widest">
                    Overall Material Tint
                  </span>
                  <div className="flex items-center gap-3">
                    <input
                      type="color"
                      value={pbrEdits.color}
                      onChange={(e) => applyColor(e.target.value)}
                      className="color-picker-input w-10 h-10 rounded-lg cursor-pointer border-2 border-white/10 hover:border-primary/40 transition-colors"
                    />
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-mono text-tech-fg uppercase">{pbrEdits.color}</span>
                      <span className="text-[9px] font-mono text-tech-muted/60">Tint multiplier</span>
                    </div>
                    <div
                      className="ml-auto w-10 h-10 rounded-lg border border-white/10"
                      style={{ backgroundColor: pbrEdits.color }}
                    />
                  </div>
                </div>

                <div className="h-px bg-tech-border/40" />
                ...
                <div className="flex flex-col gap-4">
                  <StudioSlider
                    ...
                    leftLabel="Mirror"
                    rightLabel="Matte"
                    value={pbrEdits.roughness}
                    onChange={applyRoughness}
                  ...
                  <StudioSlider
                    ...
                    leftLabel="Plastic"
                    rightLabel="Metal"
                    value={pbrEdits.metallic}
                    onChange={applyMetallic}
                    accentColor="oklch(0.65 0.18 60)"
                  />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => applyEmissive(pbrEdits.emissive, !pbrEdits.emissiveOn)}
                      className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors duration-200 cursor-pointer ${
                        pbrEdits.emissiveOn ? "bg-primary" : "bg-white/20"
                      }`}
                    >
                      <span
                        className={`inline-block h-3 w-3 rounded-full bg-white shadow-sm transform transition-transform duration-200 ${
                          pbrEdits.emissiveOn ? "translate-x-3.5" : "translate-x-0.5"
                        }`}
                      />
                    </button>
                  </div>
                  {pbrEdits.emissiveOn && (
                    <div className="flex items-center gap-3">
                      <input
                        type="color"
                        value={pbrEdits.emissive}
                        onChange={(e) => applyEmissive(e.target.value, true)}
                        className="color-picker-input w-10 h-10 rounded-lg cursor-pointer border-2 border-primary/30"
                      />
                      <div
                        className="flex-1 h-10 rounded-lg border border-white/10"
                        style={{
                          background: `radial-gradient(ellipse at center, ${pbrEdits.emissive}88 0%, transparent 70%)`,
                          backgroundColor: "#000",
                        }}
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {PRESETS.map((preset) => (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2.5 py-2 rounded-lg bg-white/5 border border-white/8 hover:bg-white/10 hover:border-primary/30 transition-all duration-150 text-left cursor-pointer group"
                      >
                        <span className="text-sm">{preset.icon}</span>
                        <span className="text-[9px] font-mono text-tech-muted group-hover:text-tech-fg transition-colors leading-tight">
                          {preset.label}
                        </span>
                      </button>
                    ))}
                ...
                <Button
                  ...
                  className="w-full gap-1.5 text-xs text-tech-muted hover:text-tech-fg border border-white/8 hover:border-white/20"
                >
              ...
              /* Empty state material settings */
              <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 py-12 text-center my-auto">
                <div className="w-12 h-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                  <Sliders className="w-5 h-5 text-primary/60" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-tech-fg mb-1">No material selected</p>
                  <p className="text-[11px] text-tech-muted leading-relaxed max-w-[200px] mx-auto">
                    Click <span className="text-primary font-mono font-bold">Pick</span>, then select any surface to adjust its overall PBR shader values
                  </p>
                </div>
              </div>
            )}
